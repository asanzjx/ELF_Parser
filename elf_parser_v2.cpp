...
int main(int argc, char* argv[]) {
	...
	if (elf_file.is_open()) {
		...
		if (ei_class == ELFCLASS32) {
			ELF32_DESC elf_desc;
			ELF32_SECTION_HEADER* section_header = (ELF32_SECTION_HEADER*)malloc(sizeof(ELF32_SECTION_HEADER));
			ELF32_PROGRAM_HEADER* program_header = (ELF32_PROGRAM_HEADER*)malloc(sizeof(ELF32_PROGRAM_HEADER));

			elf_header_parser(&elf_file, &elf_desc, &(elf_desc.elf_header));
			program_header_parser(&elf_file, &elf_desc, program_header);
			section_header_parser(&elf_file, &elf_desc, section_header);
			...
			ELF64_DESC elf_desc;
			ELF64_SECTION_HEADER* section_header = (ELF64_SECTION_HEADER*)malloc(sizeof(ELF64_SECTION_HEADER));
			ELF64_PROGRAM_HEADER* program_header = (ELF64_PROGRAM_HEADER*)malloc(sizeof(ELF64_PROGRAM_HEADER));
...
template<typename ELF_DESC,typename ELF_HEADER>ELF_HEADER* elf_header_parser(ifstream* elf_file, ELF_DESC* elf_desc, ELF_HEADER* elf_header) {
	...
	cout << "Reading header size:" << sizeof(ELF_HEADER) << " byte" << endl;
	cout << "\n****************\n" << endl;

	int i = 0;

	//read data
	elf_file->read((char*)&(elf_desc->elf_header),sizeof(ELF_HEADER));
	...
	printf("0x%0.8x:\n", offset_of(ELF_HEADER, e_machine));
	...
	printf("0x%0.8x:\n", offset_of(ELF_HEADER, e_phentsize));
	cout << "e_phentsize:0x";
	...
	printf("0x%0.8x:\n", offsetof(ELF_HEADER, e_shentsize));
	cout << "e_shentsize:0x";
	...
	printf("0x%0.8x:\n", offsetof(ELF_HEADER, e_shstrndx));
	cout << "e_shstrndx:0x";
...
template<typename ELF_DESC, typename SECTION_HEADER>SECTION_HEADER* section_header_parser(ifstream* elf_file, ELF_DESC *elf_desc, SECTION_HEADER* section_header){
	...
	cout << "[>>>] ***section header item size:" << sizeof(SECTION_HEADER) << endl;
	cout << "\n****************\n" << endl;

	// if section table exist
	if (elf_desc->elf_header.e_shoff == 0) {
	...
	for (i; i < elf_desc->elf_header.e_shnum; i++) {
		elf_file->read((char *)section_header, sizeof(SECTION_HEADER));
		elf_desc->section_header_v.push_back(*section_header);

		cout << "sh_name:";
		printf("0x%0.8x\t", elf_desc->section_header_v[i].sh_name);
		cout << "section name index";
		find_section_name<ELF_DESC>(elf_file, elf_desc, elf_desc->section_header_v[i].sh_name);
		cout << endl;
...
template<typename ELF_DESC, typename PROGRAM_HEADER>PROGRAM_HEADER* program_header_parser(ifstream* elf_file, ELF_DESC *elf_desc, PROGRAM_HEADER* program_header) {
	...
	for (i; i < elf_desc->elf_header.e_phnum; i++) {
		elf_file->read((char *)program_header, sizeof(PROGRAM_HEADER));
		elf_desc->program_header_v.push_back(*program_header);
